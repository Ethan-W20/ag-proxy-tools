...
**AG Proxy Manager** 是专为 Antigravity IDE 设计的工具集合。主要功能是允许用户在一个IDE实例中使用多个账号的额度，并可无视403身份验证直接调用账号额度，目前功能：重定向Antigravity请求地址、自动审批、上下文窗口统计、ai供应商等工具，未来会开发更多工具，让用户拥有更好的Antigravity体验
---
...
- 支持大量凭证的流式渐进加载，不阻塞首屏加载
- 可视化账号额度进度条
- 可批量查询账号额度并标记异常账号

### 🔄 多账号请求与额度调用
- 支持通过补丁重定向Antigravity请求到本地代理，再由本地代理调用不同账号向官方上游发送请求
- 支持填充模式和轮询模式去调用多个账号额度

### 📈 Token 用量统计
- 按账号跟踪输入、输出、缓存读取、缓存创建 Token 用量
- 可注入上下文统计圆环，在IDE中查看上下文大小
